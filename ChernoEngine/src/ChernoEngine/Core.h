...
#define CORE_H
  ...
  #define CE_ASSERT(x, ...) { if(!x) { CLIENTLOGGER_ERROR("Assertion failed: {0}", __VA_ARGS__); __debugbreak();) } }
  #define CE_CORE_ASSERT(x, ...) { if(!x) { CORELOGGER_ERROR("Assertion failed: {0}", __VA_ARGS__); __debugbreak(); } }
#else
  #define CE_ASSERT(x, ...)
